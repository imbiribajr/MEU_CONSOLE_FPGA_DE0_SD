...
$readelf = "F:\altera\13.0sp1\nios2eds\bin\gnu\H-i686-mingw32\bin\nios2-elf-readelf.exe"
$nm = "F:\altera\13.0sp1\nios2eds\bin\gnu\H-i686-mingw32\bin\nios2-elf-nm.exe"
...
$allowedSegments = $segments | Where-Object {
    ...
    ($_.PhysAddr -ge 0x0100C000 -and $_.PhysAddr -lt 0x01010000)
}
...
if ($contentSegments.Count -gt 8) {
    throw "Muitos segmentos ($($contentSegments.Count)) para o formato GIMG"
}
...
$entrySegment = $allowedSegments | Where-Object { $_.Flags -match "E" } | Sort-Object VirtAddr | Select-Object -First 1
if ($null -eq $entrySegment) {
    $entrySegment = $allowedSegments | Sort-Object VirtAddr | Select-Object -First 1
}
...
$bw.Write([UInt32]$entrySegment.VirtAddr)
...
Write-Host "GIMG gerado:" $outFull
