...
architecture rtl of Hardware is
    ...
    -- Sinal intermediario para os 22 bits de endereco byte do controlador
    -- bit 0  = A-1 da flash (DQ15/A-1, LSB do endereco byte em modo x8)
    -- bit 1  = A0 da flash
    -- ...
    -- bit 21 = A20 da flash
    signal tcm_addr : std_logic_vector(21 downto 0);
    ...
    component niosII_ps2 is
        port (
            ...
            generic_tristate_controller_0_tcm_write_n_out      : out   std_logic;
            generic_tristate_controller_0_tcm_read_n_out       : out   std_logic;
            generic_tristate_controller_0_tcm_chipselect_n_out : out   std_logic;
            generic_tristate_controller_0_tcm_request          : out   std_logic;
            generic_tristate_controller_0_tcm_grant            : in    std_logic                     := 'X';
            generic_tristate_controller_0_tcm_address_out      : out   std_logic_vector(21 downto 0);
            generic_tristate_controller_0_tcm_data_out         : out   std_logic_vector(7 downto 0);
            generic_tristate_controller_0_tcm_data_outen       : out   std_logic;
            generic_tristate_controller_0_tcm_data_in          : in    std_logic_vector(7 downto 0)  := (others => 'X');
...
begin
    ...
    u0 : component niosII_ps2
        port map (
            ...
            sd_spi_0_external_MISO => SD_DAT0,
            sd_spi_0_external_MOSI => SD_CMD,
            sd_spi_0_external_SCLK => SD_CLK,
            sd_spi_0_external_SS_n => SD_DAT3,
				
            -- Flash: endereco completo de 22 bits vai para sinal intermediario
            generic_tristate_controller_0_tcm_address_out      => tcm_addr,
    ...
    -- ---------------------------------------------------------------
    -- Mapeamento do endereco flash
    --
    -- O generic_tristate_controller emite 22 bits de endereco BYTE:
    --   tcm_addr(0)       = A-1  (LSB do endereco byte, pino DQ15/A-1)
    --   tcm_addr(1)       = A0
    --   tcm_addr(2)       = A1
    --   ...
    --   tcm_addr(21)      = A20
    --
    -- FL_ADDR(21:0) vai para os pinos fisicos A0..A20 da flash.
    -- FL_DQ15_AM1 vai para o pino DQ15/A-1 da flash.
    -- ---------------------------------------------------------------
    FL_DQ15_AM1 <= tcm_addr(0);           -- A-1: LSB do endereco byte
    FL_ADDR     <= '0' & tcm_addr(21 downto 1); -- A0..A20: restante do endereco
    ...
    FL_RST_N <= '1';   -- sem reset
    FL_BYTE_N <= '0';  -- modo x8 (BYTE# = LOW)
    FL_WP_N  <= '1';   -- write protect desabilitado

    -- Arbitragem: grant sempre ativo (sem outro master no barramento)
    flash_grant <= '1';
